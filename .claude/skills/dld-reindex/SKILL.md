...
```bash
bash .claude/skills/dld-reindex/scripts/find-collisions.sh --base "$BASE"
```
...
- `TAKEN` — output of `bash .claude/skills/dld-reindex/scripts/list-taken-ids.sh --base "$BASE"`. IDs already used on the base branch (and on open PRs when `gh` is available).
